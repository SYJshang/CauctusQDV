...
@implementation XRZAlertView
#pragma mark - Initializer
- (XRZAlertView *)initWithTitle:(NSString *)title message:(NSString *)message inView:(UIView *)view cancelButtonTitle:(NSString *)cancelButton confirmButton:(NSString *)confirmButton {
    if(self = [super initWithFrame:CGRectMake((ScreenW - 240) / 2,260,240,160)]) {
        _titleColor = [UIColor blueColor];
        _messageColor = HWColor(63, 203, 125);
        _confirmTextColor = [UIColor whiteColor];
        _cancelTextColor = [UIColor whiteColor];
        _buttonColor = HWColor(63, 203, 125);
        _otherButtonColor = HWColor(230, 230, 230);
        _viewColor = HWColor(154, 154, 154);
        ...
        if(cancelButton && confirmButton) {
            ...
            _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(5, CGRectGetMaxY(self.bounds) - 54, (CGRectGetWidth(self.bounds) / 2), 44)];
            _cancelButton.backgroundColor = _otherButtonColor;
            _cancelButton.tag = kCancelButtonTag;
            _cancelButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-BoldItalic" size:14];
            _cancelButton.titleLabel.adjustsFontSizeToFitWidth = YES;
            _cancelButton.titleEdgeInsets = UIEdgeInsetsMake(0, 5, 0, 5);

            _confirmButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.bounds) - (CGRectGetWidth(self.bounds) / 2), CGRectGetMaxY(self.bounds) - 54, (CGRectGetWidth(self.bounds) / 2) - 5, 44)];
            _confirmButton.backgroundColor = _buttonColor;
            _confirmButton.tag = kConfirmButtonTag;
            _confirmButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-BoldItalic" size:14];
            _cancelButton.titleLabel.adjustsFontSizeToFitWidth = YES;
...
+ (XRZAlertView *)showInView:(UIView *)view withTitle:(NSString *)title message:(NSString *)message confirmButtonTitle:(NSString *)confim cancelButtonTitle:(NSString *)cancel {
    return [[XRZAlertView alloc]initWithTitle:title message:message inView:view cancelButtonTitle:cancel confirmButton:confim];
}
...
- (void)show {
    ...
    [UIView animateWithDuration:kPresentationAnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.frame = CGRectMake((ScreenW - 240) / 2,260,240,160);
    }                completion:nil];
...
- (void)layoutSubviews {
    ...
    self.cancelButton.backgroundColor = self.otherButtonColor;
