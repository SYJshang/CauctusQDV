...
- (void)viewDidLoad {
    ...
    self.navigationItem.titleView = [UILabel titleWithColor:[UIColor whiteColor] title:@"选择照片" font:20];
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, ScreenW, ScreenH - 60) collectionViewLayout:flowOut];
...
#warning scrollView Photo
    
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    btn.frame = CGRectMake(ScreenW - 50, ScreenH - 50, 40, 40);
    ...
    self.scroll.frame = CGRectMake(0, ScreenH - 50, ScreenW - 50, 40);
    ...
    [btn addTarget:self action:@selector(actionRightBar) forControlEvents:UIControlEventTouchUpInside];
...
- (void)changeTitle{
    ...
    if (self.arrSelected.count+newSelected.count>deleteImgs.count) {
        ...
        if (self.arrSelected.count+newSelected.count == _MaxCount) {
            
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"选择图片达到最大限制" preferredStyle:UIAlertControllerStyleAlert];
            
            // Create the actions.
            UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
                XRZLog(@"The \"Okay/Cancel\" alert's cancel action occured.");
            }];
    ...
    [self.numBtn setTitle:title forState:UIControlStateNormal];
...
{
    XRZLog(@"最后一张观看的图片的index是:%zd",selecedImageViewIndex);
}
@end
